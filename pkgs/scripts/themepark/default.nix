...
let
  brightsideScript = writeShellScriptBin "brightside" ''
    if command -v emacsclient >/dev/null; then
      emacsclient -n -e "(mapc #'disable-theme custom-enabled-themes)" -e "(load-theme +default-light-theme t)" >/dev/null
    fi
  ...
  darksideScript = writeShellScriptBin "darkside" ''
    if command -v emacsclient >/dev/null; then
      emacsclient -n -e "(mapc #'disable-theme custom-enabled-themes)" -e "(load-theme +default-dark-theme t)" >/dev/null
    fi
    if command -v kitty >/dev/null; then
